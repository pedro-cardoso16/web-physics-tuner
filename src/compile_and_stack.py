...
def compile_and_stack_dataset(json_dir: str, pt_out_dir: str, max_workers: int = None):
    ...
    # -------------------------------------------------------------
    print(f"Phase 1: Compiling {len(json_paths := sorted(json_dir_path.glob('sim_*.json')))} JSON shards...")
    with ProcessPoolExecutor(max_workers) as executor:
        futures = {
            executor.submit(compile_single_shard, path, temp_pt_dir): path 
            for path in json_paths
        }
        for future in tqdm(as_completed(futures), total=len(futures), desc="Compiling Shards"):
    ...
    data_mmap = np.memmap(pt_out_path / "data.bin", dtype="float32", mode="w+", shape=(N, 7))
    hp_mmap = np.memmap(pt_out_path / "hp.bin", dtype="float32", mode="w+", shape=(N, 12))
    label_mmap = np.memmap(pt_out_path / "label.bin", dtype="float32", mode="w+", shape=(N, 2))
